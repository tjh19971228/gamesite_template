...
import { getLogoConfig, getSiteName, getSocialConfig } from "@/lib/siteConfig";
...
export function Footer({ className }: FooterProps) {
  ...
  // Social media links - only include enabled platforms
  const socialConfig = getSocialConfig();
  const allSocialLinks = [
    {
      name: "Twitter",
      href: socialConfig.twitter.link,
      enabled: socialConfig.twitter.enabled,
      icon: (
    ...
    {
      name: "Discord",
      href: socialConfig.discord.link,
      enabled: socialConfig.discord.enabled,
      icon: (
        <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
    ...
    {
      name: "YouTube",
      href: socialConfig.youtube.link,
      enabled: socialConfig.youtube.enabled,
      icon: (
        <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
  ...
  const socialLinks = allSocialLinks.filter(link => link.enabled);
      ...
      <div className="container mx-auto px-4 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-8 mb-8">
          ...
          <div className="lg:col-span-2">
              ...
              <div className="h-8 w-8 bg-primary rounded-lg flex items-center justify-center">
                {logoConfig.icon.type === "image" && logoConfig.icon.src ? (
                  <Image
                    ...
                    alt={logoConfig.icon.alt || getSiteName()}
                    width={32}
                    height={32}
                  ...
                  <svg
                    className={cn(
                      ...
                      logoConfig.icon.fallback?.className ||
                        logoConfig.icon.className
                    )}
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
            ...
            <p className="text-muted-foreground mb-4 max-w-sm">
              Your ultimate destination for free online games. Discover, play,
              and enjoy thousands of games across all genres.
            </p>
        ...
        <div className="border-t pt-8 flex flex-col sm:flex-row justify-between items-center space-y-4 sm:space-y-0">
          ...
          <div className="text-sm text-muted-foreground">
            © {currentYear} GameSite. All rights reserved. Made with ❤️ for
            gamers.
          </div>
