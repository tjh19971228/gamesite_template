...
export const GamePlayer = ({ game, isMobileView = false }: GamePlayerProps) => {
  ...
  const handleFullscreen = () => {
    ...
    try {
      ...
      } else if (gameContainerRef.current.requestFullscreen) {
    ...
    <div ref={gameContainerRef} className={gameContainerClassName}>
      ...
      {showOverlay && (
        <div className="absolute inset-0 z-10 bg-black/70 dark:bg-white/20 flex items-center justify-center">
          <button
            onClick={handlePlay}
            className="bg-primary hover:bg-primary/90 text-white w-20 h-20 md:w-24 md:h-24 rounded-full flex items-center justify-center transition-transform hover:scale-110"
