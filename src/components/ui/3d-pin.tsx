    ...
    >
      ...
      >
        <div
          ...
          className={cn(
            "absolute left-1/2 top-1/2 flex justify-start items-start rounded-2xl transition duration-700",
            className ? `${className}` : "p-4 shadow-[0_8px_16px_rgb(0_0_0/0.4)] bg-black border border-white/[0.1] group-hover/pin:border-white/[0.2] overflow-hidden"
          )}
        >
          <div className="relative z-50 w-full h-full">{children}</div>
        </div>
      </div>
      <PinPerspective title="Play Game" href={href} />
    </a>
  );
};
